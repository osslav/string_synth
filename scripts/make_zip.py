...
def  main():
  ...
  installer = "\installer\MySynth Installer.exe"
   
  if demo:
    installer = "\installer\MySynth Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\MySynth manual.pdf" 
  ]
  ...
  ZIPNAME = "MySynth-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "MySynth-v" + FULLVERSIONSTR + "-win-demo.zip"
